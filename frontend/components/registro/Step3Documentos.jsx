    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <label className="block text-sm font-semibold text-gray-800">
          ¿Cuentas con la posibilidad de emitir facturas?
          <span className="text-red-500 ml-1">*</span>
        </label>
        <p className="text-sm text-gray-600 mb-3">
          Esta información es para fines administrativos. No necesitas proporcionar tus datos fiscales en este momento.
        </p>
        <div className="flex gap-6">
          <label className="flex items-center cursor-pointer group">
            <input
              ...
              name="puede_facturar"
              value="si"
              checked={formData.puede_facturar === 'si'}
              onChange={(e) => updateFormData({ puede_facturar: e.target.value })}
              className="w-5 h-5 text-red-600 border-gray-300 focus:ring-red-500 cursor-pointer"
            />
            <span className="ml-3 text-sm font-medium text-gray-700 group-hover:text-gray-900">
              Sí, puedo emitir facturas
            </span>
          </label>
          <label className="flex items-center cursor-pointer group">
            <input
              ...
              name="puede_facturar"
              value="no"
              checked={formData.puede_facturar === 'no'}
              onChange={(e) => updateFormData({ puede_facturar: e.target.value })}
              className="w-5 h-5 text-red-600 border-gray-300 focus:ring-red-500 cursor-pointer"
            />
            <span className="ml-3 text-sm font-medium text-gray-700 group-hover:text-gray-900">
              No, no puedo emitir facturas
            </span>
          </label>
        </div>
        {errors?.puede_facturar && (
          <p className="text-sm text-red-600 mt-1">{errors.puede_facturar}</p>
        )}
      </div>
        ...
        <div className="flex">
          ...
          <div className="text-sm text-blue-800">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Tu portafolio debe mostrar obra reciente (últimos 2-3 años)</li>
              <li>Los documentos serán revisados por el equipo de curaduría</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="bg-green-50 border-l-4 border-green-600 p-4 rounded-r-2xl">
        <div className="flex">
          ...
          <div className="text-sm text-green-800">
            <p className="font-semibold mb-1">Consejos para tu portafolio:</p>
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Incluye título, dimensiones y descripción de cada obra</li>
              <li>En la siguiente sección podrás cargar tus obras para el lienzo</li>
            </ul>
          </div>
        </div>
      </div>
