...
import { ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Check, Plus, Edit2, Trash2, GripVertical, AlertCircle, Palette, Box } from 'lucide-react'
...
/**
 * Step5Paquetes - Tu Lienzo (Etapa 2)
 *
 * Diseño estilo Photoshop:
 * - Canvas SIEMPRE visible al 100%
 * - Sidebars flotan SOBRE el canvas (no lo encogen)
 */
export default function Step5Paquetes({ formData, updateFormData, errors, onContinue }) {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '75vh' }}>
      {/* CANVAS - Siempre visible, ocupa todo el espacio */}
      <div style={{
        width: '100%',
        minHeight: '70vh',
      ...
      {/* SIDEBAR IZQUIERDO - Obras (flota sobre el canvas) */}
      <div style={{
        position: 'absolute',
        top: '16px',
        left: '16px',
              ...
              {!confirmedPaquete && todasLasObras.length > 0 && (
                <div style={{ background: 'rgba(184,48,48,0.08)', borderRadius: '10px', padding: '10px 12px', marginBottom: '12px', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                  ...
                  <span style={{ fontFamily: FONTS.body, fontSize: '12px', color: COLORS.black, lineHeight: 1.4 }}>
                    Selecciona un paquete primero para arrastrar obras al lienzo
                  </span>
                </div>
              )}
              ...
              {todasLasObras.length === 0 && (
                <div style={{ textAlign: 'center', padding: '32px 16px', background: 'white', borderRadius: '12px', border: `2px dashed ${COLORS.creamDark}` }}>
                  <p style={{ fontFamily: FONTS.body, fontSize: '13px', color: COLORS.gray, margin: 0 }}>
                    No has agregado obras.<br />Haz clic en "Agregar" para comenzar.
                  </p>
                </div>
              )}
            </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '12px', background: 'rgba(184,48,48,0.05)', borderBottom: `1px solid ${COLORS.creamDark}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontFamily: FONTS.body, fontSize: '12px', color: COLORS.black }}>
                {esArtista3D ? 'Paquetes 3D (espacio base)' : 'Paquetes 2D (espacio pared)'}
              </span>
            </div>
                        ...
                        <div style={{ padding: '0 16px 16px', borderTop: `1px solid ${COLORS.creamDark}` }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
                            <span style={{ background: 'rgba(184,48,48,0.1)', padding: '6px 10px', borderRadius: '8px', fontFamily: FONTS.body, fontSize: '12px', color: COLORS.black }}>
                              {esArtista3D ? `${paquete.metros_cuadrados}m²` : `${paquete.metros_lineales}m lineales`}
                            </span>
                            <span style={{ background: 'rgba(184,48,48,0.1)', padding: '6px 10px', borderRadius: '8px', fontFamily: FONTS.body, fontSize: '12px', color: COLORS.black }}>
                              ${paquete.precio} MXN
                            </span>
                          ...
                          {!isConfirmed ? (
                            ...
                            <div style={{
                              ...
                              <Check size={16} /> Paquete Seleccionado
                            </div>
                          )}
                        </div>
                      )}
      ...
      {/* Indicador de paquete seleccionado (esquina superior) */}
      {confirmedPaquete && (
        <div style={{
          position: 'absolute',
          top: '16px',
          ...
          <button
            ...
            style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: '50%', width: '22px', height: '22px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: '4px' }}
          >
            ×
          </button>
        </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          Abre el panel de la derecha y elige el espacio para tu propuesta artística
        </p>
      </div>
    </div>
  )
      ...
      <div style={{ background: COLORS.cream, borderRadius: '20px', maxWidth: '480px', width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave({ ...obra, ...form }); }} style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <Field label="Título *" name="titulo" value={form.titulo} onChange={(e) => setForm(p => ({ ...p, titulo: e.target.value }))} required />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <Field label="Ancho (cm) *" name="ancho_cm" type="number" value={form.ancho_cm} onChange={(e) => setForm(p => ({ ...p, ancho_cm: e.target.value }))} required />
              <Field label="Alto (cm) *" name="alto_cm" type="number" value={form.alto_cm} onChange={(e) => setForm(p => ({ ...p, alto_cm: e.target.value }))} required />
            </div>
            <Field label="Técnica *" name="tecnica" value={form.tecnica} onChange={(e) => setForm(p => ({ ...p, tecnica: e.target.value }))} required />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <Field label="Año" name="anio" type="number" value={form.anio} onChange={(e) => setForm(p => ({ ...p, anio: e.target.value }))} />
              <Field label="Precio MXN *" name="precio_mxn" type="number" value={form.precio_mxn} onChange={(e) => setForm(p => ({ ...p, precio_mxn: e.target.value }))} required />
            </div>
          </div>

          <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: '14px', background: 'transparent', border: `2px solid ${COLORS.gray}`, borderRadius: '12px', fontFamily: FONTS.body, fontWeight: 600, fontSize: '14px', color: COLORS.gray, cursor: 'pointer' }}>
              Cancelar
            </button>
            <button type="submit" style={{ flex: 1, padding: '14px', background: COLORS.red, border: 'none', borderRadius: '12px', fontFamily: FONTS.body, fontWeight: 600, fontSize: '14px', color: COLORS.cream, cursor: 'pointer' }}>
              Guardar
            </button>
          </div>
        </form>
...
function Field({ label, name, value, onChange, type = 'text', required }) {
  return (
    <div>
      ...
      <input type={type} name={name} value={value} onChange={onChange} required={required} style={{
