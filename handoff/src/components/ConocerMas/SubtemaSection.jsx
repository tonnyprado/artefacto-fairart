...
function Block({ b, introRef }) {
  if (b.type === 'intro') {
    return <p ref={introRef} className={`m-0 ${cls.body}`}>{INTRO}</p>;
  }
  if (b.type === 'etimExtra') {
    return (
  ...
  if (b.type === 'h3') return <h3 className={`mb-3 ${cls.displayInk}`}>{b.text}</h3>;
  if (b.type === 'kicker') return <div className={`mb-1.5 ${cls.displayInk}`}>{b.text}</div>;
  return (
...
// Una sección = fila de grid: celda izquierda con el subtema sticky
// + celda central con etimología y bloques de contenido.
export default function SubtemaSection({ data, labelRef, introRef, minH = 'min-h-[110vh]' }) {
  return (
    <section
