...
const DEFAULT_LINKS = [
  ...
  { label: 'Contacto', href: '#contacto', pos: 'top-right' },
  { label: 'Registro', href: '#convocatoria', pos: 'bottom-left' },
  { label: 'Calendario', href: '#calendario', pos: 'bottom-right' },
];
...
export default function HeroArtefacto({ links = DEFAULT_LINKS, rows = 26, cols = 64 }) {
  ...
  const navStyle = (pos) => ({
    ...
    animation: 'flipIn 0.7s cubic-bezier(0.3,0.8,0.4,1) 2.4s both',
    ...POS_STYLE[pos],
  });
  ...
  return (
    <header id="hero" ref={heroRef} className="artefacto-hero"
      ...
      <div data-bg-letters="1" style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '0.8vh 0', overflow: 'hidden' }}>
        {bgRows.map((row, r) => (
          <div key={r} style={{ display: 'flex', gap: '0.35vh', height: '3.1vh', flex: 'none', marginLeft: row.offset, perspective: 600 }}>
            {row.letters.map((l, i) => (
              <img key={i} src={l.src} alt="" draggable={false} className="artefacto-letter"
                style={{ height: '100%', width: 'auto', flex: 'none', transition: 'opacity 0.25s', userSelect: 'none', transformOrigin: 'center bottom', animation: `flipIn 0.6s cubic-bezier(0.3,0.8,0.4,1) ${l.delay}s both` }} />
            ))}
          </div>
        ))}
      </div>
      ...
      <div data-carve="1" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(420px,42vw)', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.4vh', animation: 'flipIn 0.8s cubic-bezier(0.3,0.8,0.4,1) 2.9s both', transformOrigin: 'center bottom' }}>
