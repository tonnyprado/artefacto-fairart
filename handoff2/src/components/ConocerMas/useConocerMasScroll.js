...
export default function useConocerMasScroll(refs) {
  useEffect(() => {
    ...
    const measure = () => {
      ...
      e.labels.forEach(fit);
      fit(e.ghost);

      const labH = e.labels[0].offsetHeight;
      const pinY = L.bottom + gap;  // top del subtema fijado bajo el logo
      const lineY = pinY + labH;    // Y exacta de la línea roja compartida
      e.mask.style.height = pinY - 2 + 'px';
      e.ghost.style.transform = `translateY(${pinY}px)`;
      ...
      const lastTop = e.contents[e.contents.length - 1].getBoundingClientRect().top + window.scrollY;
    ...
    const frame = () => {
      ...
      for (let i = 0; i < n; i++) {
        const cTop = e.contents[i].getBoundingClientRect().top;
      ...
      for (let i = 0; i < n; i++) {
        e.labels[i].style.transform = `translateY(${ys[i]}px)`;
        // Expulsado del todo → ocultar (que ni un pixel itálico asome)
        e.labels[i].style.visibility = ys[i] <= g.pinY - g.labH - 2 ? 'hidden' : 'visible';
      }
    ...
    e.labels.forEach((lab, i) => lab.addEventListener('click', () => goTo(i)));
